...
type BucketValue = { bucket?: string; name?: string; value: number }
...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  const [items, setItems] = useState<LagouJob[]>([])
  ...
  const loadList = async (toPage = page, toSize = size) => {
    ...
    try {
      setLoadingList(true)
      const res = await apiFetch(`/api/lagou/list?${params.toString()}`)
      ...
      console.warn("fetch lagou list failed", e)
    } finally {
  ...
  const loadStats = async () => {
    ...
    try {
      const res = await apiFetch(`/api/lagou/stats?${params.toString()}`)
      const data: StatsResponse = await res.json()
      setStats(data)
    } catch (e) {
      console.warn("fetch lagou stats failed", e)
    }
  }
  ...
  const exportCSV = async () => {
    try {
      ...
      a.href = url
      a.download = `lagou_jobs_${new Date().toISOString().slice(0, 10)}.csv`
      a.click()
  ...
  const fallbackByDegree = useMemo(() => {
    ...
    for (const it of items) {
      const k = norm(it.degree)
      dict.set(k, (dict.get(k) || 0) + 1)
    }
    return Array.from(dict.entries()).map(([name, value]) => ({ name, value }))
  }, [items])

  // 需求变更：删去 联系人 活跃度卡片（保留相关类型定义无需使用）
    ...
    <div className="space-y-8">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input id="keyword" value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="公司/岗位/联系人" />
            </div>
            <div>
        ...
        <Card>
          ...
          <CardContent>
            {(() => {
              ...
              const labels = buckets.map(x => x.bucket || (x as BucketValue).name || "未知")
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>按筛选条件展示拉勾岗位数据</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {(items || []).map((it) => (
                  <tr key={it.id} className="border-t border-white/10">
                    <td className="py-2 px-3 whitespace-nowrap">
                      <div className="font-medium max-w-[16rem] truncate" title={it.companyName || ""}>{it.companyName || ""}</div>
                      <div className="text-xs text-muted-foreground">{it.industry || ""}</div>
                    </td>
            ...
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setDetailJob(null)}>
              <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-xl w-[800px] max-w-[90vw] p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-muted-foreground">公司：</span>{detailJob.companyName || ""}</div>
                  <div><span className="text-muted-foreground">行业：</span>{detailJob.industry || ""}</div>
                  <div><span className="text-muted-foreground">岗位：</span>{detailJob.jobTitle || ""}</div>
                  <div><span className="text-muted-foreground">薪资：</span>{detailJob.salary || ""}</div>
                  <div><span className="text-muted-foreground">城市：</span>{detailJob.location || ""}</div>
